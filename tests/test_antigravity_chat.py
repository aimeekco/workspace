...
from gws_tui.antigravity_chat import AntigravityChatAction, AntigravityChatMessage, AntigravityChatService, serialize_workspace_context
...
class AntigravityChatServiceTest(unittest.TestCase):
    def test_reply_requires_api_key(self) -> None:
        service = AntigravityChatService(antigravity_api_key="", antigravity_model="gemini-2.0-flash")

        with self.assertRaises(RuntimeError) as ctx:
            service.respond([], "What matters today?", None, None)

        self.assertIn("Antigravity disabled", str(ctx.exception))

    def test_reply_parses_text_response(self) -> None:
        service = AntigravityChatService(
        ...
        with patch("urllib.request.urlopen", return_value=urlopen_result):
            payload = service.respond(
                [AntigravityChatMessage(role="assistant", text="You have one overdue task.")],
                "What should I do first?",
                WorkspaceContext(profile_name="work", day_iso="2026-03-21"),
                TodayBrief(summary="One task and two meetings.", source="antigravity"),
            )
    ...
    def test_respond_runs_web_search_tool_and_returns_sources(self) -> None:
                ...
                json.dumps(
                    {
                        ...
                        "tool_request": {"name": "web_search", "query": "latest Antigravity model", "limit": 2},
                        "action": None,
                        "source_urls": [],
                    }
                ),
        ...
        with patch.object(service, "_call_antigravity", side_effect=lambda prompt: next(responses)) as call_mock:
            payload = service.respond([], "What's the latest Antigravity model?", None, None)

        self.assertEqual(call_mock.call_count, 2)
        self.assertEqual(search_service.calls, [("latest Antigravity model", 2)])
    ...
    def test_prompt_includes_summary_context_and_history(self) -> None:
        service = AntigravityChatService(antigravity_api_key="key", antigravity_model="gemini-2.0-flash", web_search_service=FakeWebSearchService())
        ...
        brief = TodayBrief(summary="Finish the report before noon.", warnings=["Antigravity fallback: timeout"], source="heuristic")

        prompt = service._prompt(
            [AntigravityChatMessage(role="user", text="What should I prioritize?")],
    ...
    def test_revise_after_action_error_returns_revised_action(self) -> None:
        ...
        with patch.object(
            ...
            response = service.revise_after_action_error(
                history=[AntigravityChatMessage(role="user", text="Make a project sync event.")],
                failed_action=AntigravityChatAction(kind="calendar_event_create", title="Project sync", payload={"duration_text": "30"}),
    ...
    def test_revision_prompt_includes_failed_action_and_error(self) -> None:
        ...
        prompt = service._revision_prompt(
            history=[AntigravityChatMessage(role="user", text="Make a reminder task.")],
            failed_action=AntigravityChatAction(kind="task_create", title="Reminder", payload={"title": "Reminder", "due_text": "2026-03-25"}),
    ...
    def test_parse_response_keeps_web_search_request(self) -> None:
            ...
            json.dumps(
                {
                    ...
                    "tool_request": {
                        "name": "web_search",
                        "query": "latest Antigravity model",
                        "limit": 7,
                    },
                    "action": None,
                }
        ...
        self.assertEqual(response.tool_request.query, "latest Antigravity model")
